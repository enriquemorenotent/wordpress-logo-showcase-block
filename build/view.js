...
const fadeOut = element => new Promise(resolve => transition(element, 0, resolve));
const fadeIn = element => new Promise(resolve => transition(element, 1, resolve));
const fadeOutAll = elements => Promise.all(elements.map(element => fadeOut(element)));
const fadeInAll = elements => Promise.all(elements.map(element => fadeIn(element)));
...
const configureInstance = instance => {
  ...
  const activePageElements = () => getPagedElements(images, pageSize, activePage);
  const nextActivePage = () => (activePage + 1) % Math.ceil(imageCount / pageSize);
